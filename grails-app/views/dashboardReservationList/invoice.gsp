...
<g:javascript>
    ...
    $("#flex1").flexigrid({
        url: '${resource(dir: 'dashboardReservationList', file: 'showBudgetList')}?customerId='+${params.id}+'&journalId='+${params.g}+'&bookingPeriod='+${params.m},
        dataType: 'json',
        colModel : [
    ...
    function createNewBudget(com, grid){
        url = "${resource(dir: 'budgetItemReservation', file: 'index')}?reservationId=${params.id}&journalId=${params.g}&bookingPeriod=${params.m}";
        window.location = url;
    }
    ...
    function changeBooking(incomeInvoiceId,journalId,customerId,bookingPeriod,liveUrlForEditBudgetItem){
        ...
        url = "${resource(dir: 'budgetItemReservation', file: 'list')}?editId="+incomeInvoiceId+"&journalId="+journalId+"&budgetReservationId="+customerId+"&reservationId="+customerId+"&bookingPeriod="+bookingPeriod;
        window.location = url;
    }
