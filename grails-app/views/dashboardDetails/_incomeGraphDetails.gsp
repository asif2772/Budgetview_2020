...
<%
    ...
    int budgetCustomerId = budgetCustomerArr[nTabInd][0];
    int bookInvoiceId = budgetCustomerArr[nTabInd][2]

    ArrayList glAccountArr = dashboardDetailsTagLib.getCustomerGLAccountArray(customerAccountInstance,budgetCustomerId);
    ArrayList budgetCustomerIdArr = dashboardDetailsTagLib.getBudgetCustomerIdArray(customerAccountInstance,budgetCustomerId);
    ...
    List<Map> monthlyInvoiceAmount = dashboardDetailsTagLib.getMonthlyInvoiceAmount(glAccountArr,budgetCustomerIdArr,customerInvoiceAccountInstance,false);
%>
...
<div class="grpMonthlyAmount">
    <div class="monthlyBudAmt ${(loopmonth % 2) == 0 ? 'odd' : 'even'}">
        <% if(showCountBudget > 1){
            def multipleBudgetItemsURL = liveURL + '/dashboardIncomeList/invoice/'+ budgetCustomerId + '?g='+tempGlAccount + '&m='+(monthc + 1)
        %>
        <a href="${multipleBudgetItemsURL}" target="_parent" title="Total Budget Item: ${showCountBudget}"  class="tooltip">
        <b> ${budgetAmount}</b><img width="10" height="10" alt="Edit" src="${contextPath}/images/edit.png">
        </a>
        ...
        <% } else if(showCountBudget == 1){
            def budgetIncomeURL = liveURL + '/budgetItemIncomeDetails/list' + '?editId=' + budgetItemId + '&bookingPeriod='+bookingPeriod + '&customerId='+budgetCustomerId + '&journalId='+tempGlAccount
            def toolTipText = "Total Budget Item: " + showCountBudget +" \nGL Code: " + tempGlAccount;
        %>
