...
<g:javascript>
    function loadDetailsGraph(curTabInd){
        var divId = "#reservation" + curTabInd;
        var spinnerDivId = "#spinner" + curTabInd;

        $.ajax({
                type:'POST',
                data:'curTabInd=' + curTabInd,
                url:'${context}/dashboardDetails/updateReservationGraphArea',
...
<div id="accordion" class="dbIframeBody">
    ...
    <%
        if (budgetResevationArr) {

            for (int v = 0; v < budgetResevationArr.size(); v++) {
//            int budgetVendorId = Integer.parseInt(budgetVendorArr[v][0]);
                int totalBudgetAmount = dashboardDetailsTagLib.getRoundedValue(budgetResevationArr[v][3]);
//                Double totalInvoiceAmount = dashboardDetailsTagLib.getPositiveValue(budgetResevationArr[v][4]);
                int totalInvoiceAmount = dashboardDetailsTagLib.getRoundedValue(budgetResevationArr[v][4])*(-1);
    %>
    %{--style="float:right;${(totalInvoiceAmount > totalBudgetAmount) ? 'color: #ff0000;' : 'color: #000000;'}--}%
    <h3 class="expenseHeaderStyle">
        <p class="barHeader"><g:message code="companySetup.vendorname.label" default="${budgetResevationArr[v][1]}"/></p>
    ...
    <div id="budgetId${v}" style="padding: 0px; border-radius: 0px;  margin: 0;width: 1018px; border: 1px none;">
        <div id="list-page-body-inner" class="content graphBodyInsideReserve">
            <div id="reservation${v}" style="padding: 0px; border-radius:none;">
