...
<%
    ...
    def budgetAccountCode = vendorAHWArr[nTabInd][0];

    List<Map> monthlyBudgetAmount = dashboardDetailsTagLib.getMonthlyBudgetAmountAHWise(budgetAccountCode, vendorAccountInstance);
    List<Map> monthlyInvoiceAmount = dashboardDetailsTagLib.getMonthlyInvoiceAmountAHWise(budgetAccountCode, vendorInvoiceAccountInstance,true);
%>
...
<div class="grpMonthlyAmount">
    <div class="monthlyBudAmt ${(loopmonth % 2) == 0 ? 'odd' : 'even'}">
        ...
        <% } else if (showCountBudget == 0) {
            def budgetItemURL = liveURL + '/budgetItemExpenseDetails/list' + '?bookingPeriod=' + bookingPeriod
        %>
        ...
        <% } else if (showCountBudget == 1) {
            def budgetExpenseURL = liveURL + '/budgetItemExpenseDetails/list' + '?editId=' + budgetItemId + '&bookingPeriod='+bookingPeriod + '&vendorId='+vendorId + '&journalId='+budgetAccountCode
            def toolTipText = "Total Budget Item: " + showCountBudget +" \n Vendors : " + tempVenCusName;
        %>
    ...
    <div class="monthlyInvAmt ${(loopmonth % 2) == 0 ? 'odd' : 'even'}">
        <%
                ...
                if(invOrReceipt == "invoice"){
                    expenseInvoiceURL = liveURL + '/invoiceExpense/list' + '?bookInvoiceId=' + budgetItemId + '&bookingPeriod=' + bookingPeriod + '&budgetVendorId=' + vendorId + '&vendorId=' + vendorId + '&budgetItemDetailsId=' + budgetItemDetailsId
                }else{
                    expenseInvoiceURL = liveURL + '/invoiceExpense/receiptList' + '?bookInvoiceId=' + budgetItemId + '&bookingPeriod=' + bookingPeriod + '&budgetVendorId=' + vendorId + '&vendorId=' + vendorId + '&budgetItemDetailsId=' + budgetItemDetailsId
                }
        %>
        ...
        <% } else { %>
        <a href="${expenseInvoiceURL}" target="_parent" title="Total Expense Invoice: ${showCountBudgetInvoice}"
           class="tooltip"><b>${invoiceAmount}</b>
            <img width="10" height="10" alt="Add" src="${contextPath}/images/add.png"></a></a>
        <% }
        } %>
    </div>
